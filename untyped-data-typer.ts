...
export interface ImportDirectives {
  (ctx: StructuredDataTyperContext, options: TypicalJsonTyperOptions): string[];
}

export interface TypicalJsonTyperOptions extends JsonTyperOptions {
  readonly govnDataImportURL: string;
  readonly typeImportURL: string | ImportDirectives;
...
export function defaultTypicalJsonTyperOptions(
  typeImportURL: string | ImportDirectives,
  typeName: string,
  override: Partial<
    ...
    if (isJsonTyperContext(ctx)) {
      if (uds.isFileContext(ctx.udseCtx)) {
        ...
        const modImportLines = typeof this.options.typeImportURL === "string"
          ...
          : this.options.typeImportURL(ctx, this.options).join("\n");
        textResult = `
